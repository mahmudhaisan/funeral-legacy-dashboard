...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["pending_marked_images"])) {
        ...
        $check_if_marked_images_is_in_trash_meta = (check_all_exist_in_string($marked_images, $trashed_galleries_images));

        if (!$check_if_marked_images_is_in_trash_meta) {
        ...
        $new_trashed_galleries_images = ltrim(implode(',', $new_array), ',');
        ...
        $new_pending_galleries_images = ltrim(implode(',', $new_array_merged), ',');
        ...
        $original_array = explode(',', $trashed_galleries_images);
        $new_array = array_diff($original_array, $marked_images);
        $new_trashed_galleries_images = implode(',', $new_array);
...
if ($trashed_galleries_images) {
    $attachment_ids = explode(',', $trashed_galleries_images);
    ...
    if (!empty($attachment_ids)) {
        ...
        echo '<button type="submit" name="pending_marked_images" class="btn btn-primary me-2">Move to pending list</button>';
